...
#                                   on= ['sessionid','papercode','examslot', 'centercode'])#left_on=['A_c1','c2'], right_on = ['B_c1','c2']
        ...
        superFinisher_df['eventdate_clog'] = superFinisher_df['eventdate_clog'].dt.tz_localize(None)
        superFinisher_df['eventdate_cend'] = superFinisher_df['eventdate_cend'].dt.tz_localize(None)
        superFinisher_df['end_td']=pd.to_datetime(superFinisher_df['eventdate_clog']).rsub( pd.to_datetime(superFinisher_df['eventdate_cend']) ).dt.total_seconds() / 60
...
#         db.qfinisher.drop()
        ...
        try:
                ...
                print("No Candidate gave QuickFinish")
                #statuslog(username,examstarttime, examendtime, slot, errormessage, functionname)
                error_message = "No Candidate gave QuickFinish"
                views.statuslog(un,starttime,endtime,slot,error_message,'QUICKFINISH')
