...
def checkun(request):
    if request.method == 'POST':
        ...
        try:
            exam_config = ExamConfig.objects.get(username=un)
        ...
        for script in scripttorun:
            if script  == 'quickfinish':
                try:
                    ...
                    statuslog(un,examstarttime,examendtime,slot,str(e), function_name)
            elif script == 'CSMG':
                try:
...
class ExamScript(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        new_script = Scripts.objects.filter(username=un, starttime=examstarttime, endtime=examendtime).first()

        # Prepare the response data with the newly created script details
        response_data = {
            'index': new_script.index,
            'starttime': new_script.starttime,
            'endtime': new_script.endtime,
            'slot': new_script.slot
        }
